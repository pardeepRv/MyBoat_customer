...
import firebase1 from './FirebaseConfig';
...
class FirebaseProvider {
	...
	var queryUsers = firebase1.database().ref('users');
...
DeleteAllFirebaseData=()=>{
	...
	var userRef = firebase1.database().ref('users');
	userRef.remove();
	var userMsgRef = firebase1.database().ref('message');
	userMsgRef.remove();
 }
...
messagecountforfooter=async()=>{
 ...
 getMyInboxAllData=async(userdata)=>{
	...
	var queryOffLogin = firebase1.database().ref('users/').child(id).child('/myInbox/');
		queryOffLogin.off('child_added');
	
		
	//---------------------------- inbox get first time all ---------------
	var query = firebase1.database().ref('users/'+id+'/myInbox/');
	...
	//--------------------------------- update code --------------
	var queryUpdate = firebase1.database().ref('users/'+id+'/myInbox/');
...
CreateUser=(id, jsonUserData)=>{
        ...
        firebase1.database().ref('users/' + id).update(jsonUserData).then(()=> {
        consolepro.consolelog("CreateUser success.");
        var onlineStatusRef = firebase1.database().ref('users/'+id+'/onlineStatus/');
...
firebaseUserGetInboxCount=async(userdata)=>{
	...
	// var user_arr = await localStorage.getItemObject('user_arr');
	   var user_arr  =userdata;
		...
		count_inbox=0;
			...
			var queryAllUser = firebase1.database().ref('users/'+id+'/myInbox');
	 ...
	 firebaseUserCreateUpdatePlayerId=async()=>{
       ...
       var user_id= user_arr.user_id;
		...
		var queryAllUser = firebase1.database().ref('users');
		  queryAllUser.once('value', (data)=> {
			  consolepro.consolelog('user all data',data.toJSON());
			  var allUserArr=Object.values(data.toJSON());
			  consolepro.consolelog('allUserArr',allUserArr);
	...
	// 	}
	   CreateUserInboxOther=async(id, jsonUserData)=>{
		...
		firebase1.database().ref().child('users/'+id+'/myInbox/').update(jsonUserData).then(function() {
	 ...
	 UpdateUserInboxMe=(id, otherId, jsonUserData)=>{
		...
		firebase1.database().ref('users/' + id+'/myInbox/'+otherId).update(jsonUserData).then(function() {
			consolepro.consolelog("Update inbox succeeded.");
	  })
	  .catch(function(error) {
		consolepro.consolelog("Update Inbox failed: " + error.message);
	...
	SendUserMessage=(messageId, messageJson, messageType, inputId)=> {
		...
		firebase1.database().ref('message' + '/' + messageId).push().set(messageJson).then(function() {
 ...
 UpdateUserInboxOther=(id, otherId, jsonUserData2)=>{
		...
		firebase1.database().ref('users/' + id+'/myInbox/'+otherId).update(jsonUserData2).then(()=> {
		consolepro.consolelog("Update inbox succeeded.");
	  })
	  .catch((error)=> {
		consolepro.consolelog("Update Inbox failed: " + error.message);
